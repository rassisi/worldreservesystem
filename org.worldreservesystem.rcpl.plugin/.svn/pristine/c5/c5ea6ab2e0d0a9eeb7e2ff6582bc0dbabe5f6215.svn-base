 ...
 * @param <T>
 */
public class JOPropertyTopInset<T extends Double> extends
		JOPropertyInteger<Double> implements IObverserObvervable {

	/**
	 * @param xmlObject
	 */
	public JOPropertyTopInset(JOObject layoutObject, XmlObject xmlParent,
			XmlObject xmlObject) {
	...
	public Double get() {
		if (data == null) {
			if (xmlObject instanceof CTTextBodyProperties) {
				data = ((CTTextBodyProperties) xmlObject).getTIns();
			}
		}
		if (data == null) {
