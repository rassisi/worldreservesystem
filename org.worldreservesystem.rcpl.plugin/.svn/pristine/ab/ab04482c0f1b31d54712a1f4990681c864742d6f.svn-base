 ...
 * @param <T>
 */
public class JOPropertyBottomInset<T extends Double> extends
		JOPropertyInteger<Double> implements IObverserObvervable {

	/**
	 * @param xmlObject
	 */
	public JOPropertyBottomInset(JOObject layoutObject, XmlObject xmlParent,
			XmlObject xmlObject) {
	...
	public Double get() {
		if (data == null) {
			if (xmlObject instanceof CTTextBodyProperties) {
				data = ((CTTextBodyProperties) xmlObject).getBIns();
			}
		}
		if (data == null) {
