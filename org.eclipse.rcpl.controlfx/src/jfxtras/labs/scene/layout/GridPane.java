...
/**
 * A drop-in replacement for JavaFX's GridPane using layout constraints for the
 * nodes. You should still use the RowConstraints(Builder) and
 * ColumnConstraints(Builder)
 * 
 * So instead of writing: Text t = new Text("Services"); grid.add(t, 3, 2);
 * GridPane.valignment(t, VPos.TOP));
 * 
 * You can write: grid.add(new Text("Services"), new
 * GridPane.C().col(3).row(2).valignment(VPos.TOP));
 *
 * This class is not a reimplementation of GridPane, but only applies a
 * different API.
 * 
 * @author Tom Eugelink
 *
...
public class GridPane extends javafx.scene.layout.GridPane {
	...
	public static class C extends GenericLayoutConstraints.C<C> {
		...
		// margin
		public C margin(javafx.geometry.Insets value) {
			this.margin = value;
			return this;
		}
		...
		// halignment
		public C halignment(javafx.geometry.HPos value) {
		...
		// hgrow
		public C hgrow(javafx.scene.layout.Priority value) {
			this.hgrow = value;
			return this;
		}
		...
		// valignment
		public C valignment(javafx.geometry.VPos value) {
		...
		// hgrow
		public C vgrow(javafx.scene.layout.Priority value) {
			this.vgrow = value;
			return this;
		}
	 ...
	 */
	private WeakHashMap<Node, C> cMap = new WeakHashMap<Node, C>();
	...
	/**
	 * set constraint without adding the node (in case the node might end up
	 * here because of an animation or something)
	 */
