 ...
 * </pre>
 * 
 * It reproduces a problem I experience inside an application I am writing. If
 * you magnify to {@link #MAX_SCALE}, an interesting problem occurs when you try
 * to zoom back to {@link #MIN_SCALE}. In the beginning you will see that the
 * {@code scrollPane} scrolls and consumes the {@code ScrollEvent} until we have
 * scrolled to the bottom of the window. Once the bottom of the window is
 * reached, it behaves as expected (or at least as I was expecting).
 *
...
public class ZoomApp extends Application {
	...
	private class ZoomHandler implements EventHandler<ScrollEvent> {
		...
		private double calculateScale(ScrollEvent scrollEvent) {
			double scale = nodeToZoom.getScaleX() + scrollEvent.getDeltaY()
					/ 100;
	...
	private Group createSixRectangles() {
		return new Group(createRectangle(0, 0), createRectangle(110, 0),
				createRectangle(220, 0), createRectangle(0, 110),
				createRectangle(110, 110), createRectangle(220, 110),
				createRectangle(0, 220), createRectangle(110, 220),
				createRectangle(220, 220));
	}
