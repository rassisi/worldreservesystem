...
public final class ZipPackage extends Package {
	...
	/**
	 * Zip archive, as either a file on disk, or a stream
	 */
	...
	/**
	 * Constructor. Opens a Zip based Open XML document from an InputStream.
	 *
	 * @param in
	...
	/**
	 * Constructor. Opens a Zip based Open XML document from a custom
	 * ZipEntrySource, typically an open archive from another system
	 *
	 * @param zipEntry
	 *            Zip data to load.
	 * @param access
	...
	protected PackagePart[] getPartsImpl() throws InvalidFormatException {
		...
		while (entries.hasMoreElements()) {
			...
			if (contentType != null && contentType.equals(ContentTypes.RELATIONSHIPS_PART)) {
				try {
					partList.put(partName, new ZipPackagePart(this, entry, partName, contentType));
		...
		while (entries.hasMoreElements()) {
			...
			if (contentType != null && contentType.equals(ContentTypes.RELATIONSHIPS_PART)) {
				...
				throw new InvalidFormatException("The part " + partName.getURI().getPath()
	...
	/**
	 * Builds a PackagePartName for the given ZipEntry, or null if it's the
	 * content types / invalid part
	 */
	private PackagePartName buildPartName(ZipEntry entry) {
		try {
			...
			logger.log(POILogger.WARN,
					"Entry " + entry.getName() + " is not valid, so this part won't be add to the package.", e);
			return null;
		}
	}
	...
	protected void closeImpl() throws IOException {
		...
		if (this.originalPackagePath != null && !"".equals(this.originalPackagePath)) {
			...
			if (targetFile.exists()) {
				...
				File tempFile = TempFile.createTempFile(generateTempFileName(FileHelper.getDirectory(targetFile)),
						".tmp");
				...
				try {
					...
					// temporary file
					if (!tempFile.delete()) {
						logger.log(POILogger.WARN, "The temporary file: '" + targetFile.getAbsolutePath()
	...
	public void saveImpl(OutputStream outputStream) {
		...
		try {
			...
			if (this.getPartsByRelationshipType(PackageRelationshipTypes.CORE_PROPERTIES).size() == 0
				...
				this.relationships.addRelationship(this.packageProperties.getPartName().getURI(), TargetMode.INTERNAL,
			...
			for (PackagePart part : getParts()) {
				...
				logger.log(POILogger.DEBUG,
						"Save part '" + ZipHelper.getZipItemNameFromOPCName(part.getPartName().getName()) + "'");
