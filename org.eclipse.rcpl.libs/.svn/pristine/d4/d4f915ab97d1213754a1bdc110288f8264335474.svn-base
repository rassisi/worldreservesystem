...
/**
 * Represents a metafile picture which can be one of the following types: EMF,
 * WMF, or PICT. A metafile is stored compressed using the ZIP deflate/inflate
 * algorithm.
 *
 * @author Yegor Kozlov
 */
public abstract class Metafile extends HSLFPictureData {
	...
	/**
	 * A structure which represents a 34-byte header preceeding the compressed
	 * metafile data
	 *
	 * @author Yegor Kozlov
	 */
	public static class Header {
		...
		public void read(byte[] data, int offset) {
			...
			compression = LittleEndian.getUByte(data, pos);
			pos++;
			filter = LittleEndian.getUByte(data, pos);
			pos++;
		}
	...
	public Dimension getImageDimension() {
		...
		return new Dimension((int) Math.round(Units.toPoints((long) header.size.getWidth())),
