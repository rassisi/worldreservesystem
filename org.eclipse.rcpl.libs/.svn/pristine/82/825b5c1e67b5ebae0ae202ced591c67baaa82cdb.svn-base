...
public final class WMF extends Metafile {
	...
	public void setData(byte[] data) throws IOException {
		...
		header.bounds = new Rectangle((short) nHeader.left, (short) nHeader.top,
				(short) nHeader.right - (short) nHeader.left, (short) nHeader.bottom - (short) nHeader.top);
	...
	public static class NativeHeader {
		...
		public NativeHeader(byte[] data, int pos) {
			...
			left = LittleEndian.getShort(data, pos);
			pos += LittleEndian.SHORT_SIZE;
			top = LittleEndian.getShort(data, pos);
			pos += LittleEndian.SHORT_SIZE;
			right = LittleEndian.getShort(data, pos);
			pos += LittleEndian.SHORT_SIZE;
			bottom = LittleEndian.getShort(data, pos);
			pos += LittleEndian.SHORT_SIZE;
		 ...
		 * The checksum is calculated by XORing each short value to an initial
		 * value of 0:
		 */
