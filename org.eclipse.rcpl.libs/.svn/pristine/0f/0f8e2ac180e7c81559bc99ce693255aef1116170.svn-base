...
public class SpellCheckEngine implements ISpellCheckEngine,
	...
	public ISpellDictionary findDictionary(Locale locale) {
		ISpellDictionary dictionary = (ISpellDictionary) fLocaleDictionaries
				.get(locale);
		if (dictionary != null)
			return dictionary;
...
//		return getResources("/" + DICTIONARY_LOCATION); //$NON-NLS-1$
	 ...
	 * getSpellChecker()
	 */
	public synchronized final ISpellChecker getSpellChecker()
		...
		Locale locale = Locale.getDefault(); // getCurrentLocale(store);
	...
	public final void propertyChange(final PropertyChangeEvent event) {
		...
		// if
		// (event.getProperty().equals(SpellingService.PREFERENCE_SPELLING_ENABLED)
		// &&
	...
	private synchronized void resetUserDictionary() {
		...
		// variablesPlugin.getStringVariableManager();
		// try {
