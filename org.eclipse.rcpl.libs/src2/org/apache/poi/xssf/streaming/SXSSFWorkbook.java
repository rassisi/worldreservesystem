 ...
 * has to be kept in memory. Depending on your document content this could use a
 * lot more resources than with shared strings disabled.
 *
...
public class SXSSFWorkbook implements Workbook {
	/**
	 * Specifies how many rows can be accessed at most via getRow(). When a new
	 * node is created via createRow() and the total number of unflushed records
	 * would exeed the specified value, then the row with the lowest index value
	 * is flushed and cannot be accessed via getRow() anymore.
	 */
	 ...
	 * <ol>
	 * <li>Append new sheets to existing workbooks. You can open existing
	 * workbook from a file or create on the fly with XSSF.</li>
	 * <li>Append rows to existing sheets. The row number MUST be greater than
	 * max(rownum) in the template sheet.</li>
	 * <li>Use existing workbook as a template and re-use global objects such as
	 * cell styles, formats, images, etc.</li>
	 * </ol>
	 * All three use cases can work in a combination.
	 * </p>
	 * What is not supported:
	 * <ul>
	 * <li>Access initial cells and rows in the template. After constructing
	 * SXSSFWorkbook(XSSFWorkbook) all internal windows are empty and
	 * SXSSFSheet@getRow and SXSSFRow#getCell return null.</li>
	 * <li>Override existing cells and rows. The API silently allows that but
	 * the output file is invalid and Excel cannot read it.</li>
	 * </ul>
	 *
	 * @param workbook
	 ...
	 * unflushed records would exceed the specified value, then the row with the
	 * lowest index value is flushed and cannot be accessed via getRow()
	 * anymore.
	 * </p>
	 * <p>
	 * A value of -1 indicates unlimited access. In this case all records that
	 * have not been flushed by a call to flush() are available for random
	 * access.
	 * <p>
	 * <p>
	 * </p>
	 ...
	 * <p>
	 * SXSSF writes sheet data in temporary files (a temp file per-sheet) and
	 * the size of these temp files can grow to to a very large size, e.g. for a
	 * 20 MB csv data the size of the temp xml file become few GB large. If the
	 * "compress" flag is set to <code>true</code> then the temporary XML is
	 * gzipped.
	 * </p>
	 * <p>
	...
	void deregisterSheetMapping(XSSFSheet xSheet) {
		...
		// ensure that the writer is closed in all cases to not have lingering
		// writers
		try {
	...
	private static void copyStreamAndInjectWorksheet(InputStream in, OutputStream out, InputStream worksheetData)
			throws IOException {
		InputStreamReader inReader = new InputStreamReader(in, "UTF-8"); // TODO:
																			// Is
																			// it
																			// always
																			// UTF-8
		...
		String s = "<sheetData";
		int n = s.length();
		// Copy from "in" to "out" up to the string "<sheetData/>" or
		// "</sheetData>" (excluding).
	...
	/**
	 * Create an Sheet for this Workbook, adds it to the sheets and returns the
	 * high level representation. Use this to create new sheets.
	 *
	 * @param sheetname
	 ...
	 * 
	 * @deprecated use {@link SXSSFSheet#setRepeatingRows(CellRangeAddress)} or
	 *             {@link SXSSFSheet#setRepeatingColumns(CellRangeAddress)}
	 */
	@Deprecated
	@Override
	...
	/**
	 * Closes the underlying {@link XSSFWorkbook} and {@link OPCPackage} on
	 * which this Workbook is based, if any. Has no effect on Workbooks created
	 * from scratch.
	 */
	@Override
	...
	/**
	 * Dispose of temporary files backing this workbook on disk. Calling this
	 * method will render the workbook unusable.
	 * 
	 ...
	 * 
	 * @param sheetIndex
	 *            Zero-based sheet index (0 Represents the first sheet to keep
	 ...
	 * various instances one needs for HSSF, XSSF and SXSSF.
	 */
	@Override
	...
	/**
	 * @param hiddenFlag
	 *            pass <code>false</code> to make the workbook visible in the
	 *            GUI
	 */
	@Override
	 ...
	 *            the sheet index (0-based)
	 * @param hidden
	 *            one of the following <code>Workbook</code> constants:
	 ...
	 * 0, this flag will tell Excel that it needs to recalculate all formulas in
	 * the workbook the next time the file is opened.
	 * </p>
	 *
	 * @param value
