...
public class SpellCheckIterator implements ISpellCheckIterator {
	...
	public SpellCheckIterator(IDocument document, IRegion region,
		...
		try {
			...
			if (content.startsWith(TAG_PREFIX))
				content = ""; //$NON-NLS-1$
	...
	protected String nextToken() {
		...
		if (fNext - fPrevious > 0) {
			...
			// if (fSuccessor != DONE && fContent.charAt(fPrevious) ==
			// IJavaDocTagConstants.JAVADOC_TAG_PREFIX) {
			//
			// nextBreak();
			...
			if (fSuccessor != BreakIterator.DONE
					&& fContent.charAt(fPrevious) == IHtmlTagConstants.HTML_TAG_PREFIX
					&& (Character.isLetter(fContent.charAt(fNext)) || fContent
							.charAt(fNext) == '/')) {

				if (fContent.startsWith(IHtmlTagConstants.HTML_CLOSE_PREFIX,
						fPrevious))
					nextBreak();
