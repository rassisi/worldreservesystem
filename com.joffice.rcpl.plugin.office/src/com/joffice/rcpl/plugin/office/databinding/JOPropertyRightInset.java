 ...
 * @param <T>
 */
public class JOPropertyRightInset<T extends Double> extends
		JOPropertyInteger<Double> implements IObverserObvervable {

	/**
	 * @param xmlObject
	 */
	public JOPropertyRightInset(JOObject layoutObject, XmlObject xmlParent,
			XmlObject xmlObject) {
	...
	public Double get() {
		if (data == null) {
			if (xmlObject instanceof CTTextBodyProperties) {
				data = ((CTTextBodyProperties) xmlObject).getRIns();
			}
		}
		if (data == null) {
